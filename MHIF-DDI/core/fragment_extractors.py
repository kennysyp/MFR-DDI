...
def k_hop_fragment(edge_index, num_nodes, num_hops, is_directed=False):
...
def random_fragment(g, n_patches, num_hops=1):
...
def metis_fragment(g, n_patches, drop_rate=0.0, num_hops=1, is_directed=False):
    if is_directed:
    ...
    if num_hops > 0:
        fragments_batch, fragments_node_mapper = node_mask.nonzero().T
        k_hop_node_mask = k_hop_fragment(
            g.edge_index, g.num_nodes, num_hops, is_directed)
        node_mask.index_add_(0, fragments_batch,
                             k_hop_node_mask[fragments_node_mapper])
