...
def evaluate(model, loader, set_len, criterion, args):
    ...
    for batch in loader:
        ...
        test_loss += loss.item() * graph_batch_1.num_graphs
        N += graph_batch_1.num_graphs
    ...
    return acc, auc, f1, p, r, ap, test_loss / N
...
def train(model, args):
    if args.dataset == "drugbank":
        ...
        test_set = DDIDataset(args.dataset, "test", args.fold,args.inductive)
    ...
    scheduler = torch.optim.lr_scheduler.LambdaLR(
        optimizer,
        lambda epoch: (1.0 if epoch < 50 else 0.1),
    ...
    for epoch in range(args.num_epoch):
        ...
        for i, batch in enumerate(train_loader):
            ...
            y_pred = model.forward_func(graph_batch_1, graph_batch_2, ddi_type)
            loss = criterion(y_pred, y_true)
            train_loss += loss.item() * graph_batch_1.num_graphs
            N += graph_batch_1.num_graphs
        ...
        test_acc, test_auc, test_f1, test_p, test_r, test_ap, test_loss = \
            evaluate(model, test_loader, test_set_len, criterion, args)
        print()
        print(test_loss)
        print(
        ...
        if valid_acc > max_valid_acc:
            ...
            torch.save(model.state_dict(), f"./model_{args.start_epoch + epoch}.pt")
